...
const InventoryInfoDashboardNew = ({ inventoryData }) => {
  const [showPhotoModal, setShowPhotoModal] = useState(false);
  const [selectedPhoto, setSelectedPhoto] = useState(null);
  const [selectedSpan, setSelectedSpan] = useState("");
  const photos = inventoryData?.photos || [];
  const spanphotos = inventoryData?.images_spans || [];
  const spanIndexes = Array.from({ length: inventoryData?.no_of_span || 0 }, (_, i) => i + 1);
  ...
  return (
    <div className="container">
      <div className="card p-3" style={{ maxWidth: "900px", width: "100%", border: "2px solid #60A5FA", boxShadow: "0 4px 8px rgba(0, 0, 0, 0.2)" }}>
        ...
        <Form>
            ...
            {[
              ...
              <Col key={index} md={6} lg={4}>
                <Form.Group>
                  ...
                  <Form.Control
                    type="text"
                    value={field.includes(",") ? field.split(",").map(f => inventoryData[f.trim()] || "").join(", ") : inventoryData?.[field] || ""}
                    readOnly
          ...
          <Form.Group>
            ...
            <Form.Select value={selectedSpan} onChange={handleSpanSelect} style={{ padding: "8px", fontSize: "14px" }}>
              <option value="">-- Select Span --</option>
              {spanIndexes.map((span) => (
                <option key={span} value={span}>Span {span}</option>
              ))}
            </Form.Select>
          </Form.Group>
      ...
      <Modal show={showPhotoModal} onHide={closeModal} centered>
        ...
        <Modal.Body className="text-center">
          {selectedPhoto && <img src={selectedPhoto} alt="Selected Photo" className="img-fluid" style={{ maxHeight: "400px", objectFit: "contain" }} />}
        </Modal.Body>
      </Modal>
    </div>
  );
...
export default InventoryInfoDashboardNew;