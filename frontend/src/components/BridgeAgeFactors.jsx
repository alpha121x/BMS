...
const BridgeAgeFactors = () => {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${BASE_URL}/api/age-factors`);
  ...
  return (
    ...
    >
      <div className="card-body pb-0">
        <h6 className="card-title text-lg font-semibold pb-2">Bridge Age Factors</h6>
            ...
            {currentData.length > 0 ? (
              currentData.map((row, index) => (
                <tr key={index}>
                  ...
                  <td>{row.BridgeAgeFactorID || "N/A"}</td>
                  <td>{row.BridgeAgeFactorName || "N/A"}</td>
                  <td>{row.BridgeAgeFactorValue || "0"}</td>
