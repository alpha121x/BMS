...
app.post("/api/bridge-status-summary-all", async (req, res) => {
  try {
    ...
    // Base query for bridge status summary
    let summaryQuery = `
      SELECT 
        t.uu_bms_id,
        CONCAT(m.pms_sec_id, ',', m.structure_no) AS bridge_name,
    ...
    // Base query for unapproved inspections by Consultant
    let consultantQuery = `
    ...
    // Base query for unapproved inspections by RAMS
    let ramsQuery = `
      SELECT DISTINCT ON (ins.uu_bms_id)
        bmd."pms_sec_id", 
        bmd."structure_no",
    ...
    if (districtId && districtId !== "%") {
      conditionsSummary.push(`m.district_id = $${paramIndex}`);
      conditionsBmd.push(`bmd.district_id = $${paramIndex}`);
      values.push(districtId);
      paramIndex++;
    }

    if (bridgeName && bridgeName.trim() !== "" && bridgeName !== "%") {
      conditionsSummary.push(
        `LOWER(CONCAT(m.pms_sec_id, ',', m.structure_no)) ILIKE $${paramIndex}`
      );
      conditionsBmd.push(
        `LOWER(CONCAT(bmd.pms_sec_id, ',', bmd.structure_no)) ILIKE $${paramIndex}`
      );
      values.push(`%${bridgeName.toLowerCase()}%`);
      paramIndex++;
    }
    ...
    if (structureType && structureType !== "%") {
      conditionsSummary.push(`m.structure_type_id = $${paramIndex}`);
      conditionsBmd.push(`bmd.structure_type_id = $${paramIndex}`);
      values.push(structureType);
      paramIndex++;
    }

    if (conditionsSummary.length > 0) summaryQuery += ` AND ${conditionsSummary.join(" AND ")}`;
    if (conditionsBmd.length > 0) {
      consultantQuery += ` AND ${conditionsBmd.join(" AND ")}`;
      ramsQuery += ` AND ${conditionsBmd.join(" AND ")}`;
    }
    ...
    consultantQuery += ` ORDER BY ins.uu_bms_id, ins.inspection_id DESC;`;
    ramsQuery += ` ORDER BY ins.uu_bms_id, ins.inspection_id DESC;`;

    // Log queries with parameters
    logQuery(`Summary Query: ${summaryQuery} | Parameters: ${JSON.stringify(values)}`);
    logQuery(`Consultant Query: ${consultantQuery} | Parameters: ${JSON.stringify(values)}`);
    logQuery(`RAMS Query: ${ramsQuery} | Parameters: ${JSON.stringify(values)}`);

    // Run queries in parallel
    const [summaryResult, consultantResult, ramsResult] = await Promise.all([
      pool.query(summaryQuery, values),
      pool.query(consultantQuery, values),
      pool.query(ramsQuery, values),
    ]);

    // --- Process PhotoPaths ---
    const processPhotoPaths = (rows) => {
      return rows.map((row) => {
    ...
    const consultantData = processPhotoPaths(consultantResult.rows);
    const ramsData = processPhotoPaths(ramsResult.rows);

    // --- Combine Final Data ---
    const combinedData = summaryResult.rows.map((bridge) => {
      const consultantUnapproved = consultantData.filter(
        (item) => item.uu_bms_id === bridge.uu_bms_id
      );
      const ramsUnapproved = ramsData.filter(
        (item) => item.uu_bms_id === bridge.uu_bms_id
      );
    ...
    console.error("Error fetching bridge inspections:", error);
    logQuery(`Error: ${error.message}`);
...
app.get("/api/inspections", async (req, res) => {
  try {
    let { district, bridge, uu_bms_id } = req.query;
...
// ✅ Get past overall conditions
app.get("/api/past-overall-conditions", async (req, res) => {
  const { uu_bms_id } = req.query;

  if (!uu_bms_id) {
    return res.status(400).json({ error: "Missing uu_bms_id" });
  }
